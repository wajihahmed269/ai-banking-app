...
public class JwtService {
    ...
    public JwtService(
        ...
        String secret = configuredSecret == null || configuredSecret.isBlank()
                ? generateLocalDevelopmentSecret()
                : configuredSecret;
        this.signingKey = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8));
    }

    private String generateLocalDevelopmentSecret() {
        byte[] bytes = new byte[32];
        SECURE_RANDOM.nextBytes(bytes);
        log.warn("JWT_SECRET is not configured. Using an ephemeral local-development signing key; "
                + "configure JWT_SECRET for production and Kubernetes deployments.");
